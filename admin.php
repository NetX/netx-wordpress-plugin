...
class netxMediaManagerPluginAdmin {
    ...
    public function registerSettings() {
        ...
        add_settings_field('netx_uri_protocal', 'NetX URI Protocal', array($this, 'netxURIProtocal'), __FILE__, 'main_section');
        add_settings_field('netx_paging_size', 'NetX Media Page Size', array($this, 'netxPageSize'), __FILE__, 'main_section');


        add_settings_field('netx_base_category_id', 'NetX Folder', array($this, 'netxCategory'), __FILE__, 'main_section');
    ...
    /**
     * Create NetX URI Protocl field
     */
    public function netxURIProtocal() {
        $options = get_option('netx_options');
        echo '<p>Should the netx api be contacted over Https? (HTTPS is strongly recommended)</p>';
        echo '<select name="netx_options[netx_uri_protocal]">';
        echo '<option value="http"' . ($options['netx_uri_protocal'] === "http" ? ' selected="selected"' : '') . '>http</option>';
        echo '<option value="https"' . ($options['netx_uri_protocal'] !== "http" ? ' selected="selected"' : '') . '>https</option>';
        echo '</select>';
    }
    ...
    public function netxCategory() {
        $options = get_option('netx_options');
        echo '<input type="number" name="netx_options[netx_base_category_id]" value="' . $options['netx_base_category_id'] . '" />';
    }
    ...
    public function netxCategoryPath(){
        $options = get_option('netx_options');
        echo '<p>Current Folder Path: ' . $options['netx_base_category_path'] . '</p>';
